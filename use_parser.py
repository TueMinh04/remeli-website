    ...
    def handle_data(self, data):
        ...
        if data_stripped:
            for old, key in self.replacements:
                # To handle slight whitespace differences:
                if re.sub(r'\s+', ' ', old.strip()) == re.sub(r'\s+', ' ', data_stripped):
                    # Found it! We need to inject data-i18n into the PREVIOUS start tag in self.output
                    # The previous start tag is typically the last item in self.output that starts with '<' and doesn't start with '</'
                    for i in range(len(self.output)-1, -1, -1):
                        if self.output[i].startswith('<') and not self.output[i].startswith('</'):
                            # Insert data-i18n right after the tag name
                            # e.g., <span class="..."> -> <span data-i18n="key" class="...">
...
about = [
    ...
    ("Tại Việt Nam, hàng tấn thức ăn hoàn toàn ngon lành bị lãng phí mỗi ngày chỉ vì chưa tìm được người dùng kịp thời. Remeli ra đời với một khát vọng đơn giản: tạo ra một nền tảng kết nối các cửa hàng thực phẩm với những người yêu ẩm thực, giúp", "aboutVisionDesc_p1"),
    ("cứu lấy những bữa ăn ngon", "aboutVisionDesc_p2"),
    (", tiết kiệm chi phí, và quan trọng nhất là", "aboutVisionDesc_p3"),
    ("giảm thiểu lượng khí thải carbon", "aboutVisionDesc_p4"),
    ("gây hiệu ứng nhà kính từ rác thải thực phẩm.", "aboutVisionDesc_p5"),
...
process('about-remeli.html', about)
...
process('phan-hoi-khach-hang.html', feedback)
