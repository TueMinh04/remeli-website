...
def rep(filename, replacements):
    ...
    for text, key in replacements:
        # Regex explanation:
        # (<[a-zA-Z0-9_-]+[^>]*)> : matches the opening tag, including any existing attributes, up to the closing >
        # (\s*) : matches optional whitespace before the text
        # re.escape(text) : matches the exact text
        # (\s*<) : matches optional whitespace after the text and the < of the closing tag or next tag
        
        escaped_text = re.escape(text).replace(r'\ ', r'\s+').replace(r'\n', r'\s+')
        pattern = re.compile(r'(<[a-zA-Z0-9_-]+[^>]*)>\s*' + escaped_text + r'\s*<', re.DOTALL)
        
        def replacer(match):
            tag_start = match.group(1)
            # If the tag already has data-i18n, we skip adding another one (or maybe we replace it? Let's just append if it doesn't have it)
            if 'data-i18n=' in tag_start:
                return match.group(0) # Do nothing
            return f'{tag_start} data-i18n="{key}">' + match.group(0)[len(match.group(1))+1:]

        new_content = pattern.sub(replacer, content)
        if new_content == content:
            print(f"WARNING: '{text[:30]}...' not found in {filename}")
        content = new_content
...
rep('lam-sao-de-pick-up.html', pickup)
...
rep('phan-hoi-khach-hang.html', feedback)
